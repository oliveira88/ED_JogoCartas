...
void main() {
    ...
    Lista *listaInicio;
    Recursos recursos;
    Tarefa *tarefas, *tarefasCumpridas, *tarefasNaoCumpridas;
    Pilha *monte, *descarte;
    Arvore *arvoreDescartadas;
    int nivel;
    int n = -1;
    int turnoAtual = 1;
    ...
    listaInicio = NULL;
    tarefas = NULL;
    tarefasCumpridas = NULL;
    tarefasNaoCumpridas = NULL;
    monte = NULL;
    descarte = NULL;
    arvoreDescartadas = NULL;
    ...
    while (n != 0) {
        ...
        if (n == 2) {
            ...
            Lista *auxiliar = listaInicio;
            for (int i = 0; i < qtdDescarte; i++)
            {
                if (auxiliar != NULL)
                {
                    insereArvore(&arvoreDescartadas, auxiliar->carta);
        ...
        if (n == 3) {
            cumpreTarefas(&recursos, &tarefas, &tarefasCumpridas, turnoAtual);
    ...
    printf("\nProfundidade minima da arvore: %d\n", calculaMenor(&arvoreDescartadas));
    printaPontuacao(recursos, tarefasCumpridas, tarefasNaoCumpridas, calculaMenor(&arvoreDescartadas));
    fclose(arqCartas);
    freeLista(&listaInicio);
    freeTarefas(&tarefas);
    freePilha(&monte);
